...
CAROUSEL FORMAT (required for this batch): make "onscreen_text" a set of 5–8 numbered slide lines, each on its own line ("Slide 1: ...", "Slide 2: ..."), each slide pulling to the next, the last slide a mic drop + the CTA per the OFFERS law (this account's own keyword — never a generic "Comment WISH"). These lines are the skeleton Mandi builds in Canva.` : ''
    ...
    try {
      const hookSys = 'You are Claude Fable writing the two hardest lines of a social post: the scroll-stopping on-screen hook and the caption\'s first sentence (a DIFFERENT door — never the same words). Both TALK TO HER — second person ("you", "your") — a hand on her elbow, not a diary and not a label. BANNED: describing her scene back at her ("It\'s 9pm and you\'re watching your fourth tutorial", "You have 23 tabs open") = performed empathy, she scrolls. ALSO BANNED: a first-person memoir ("I saved it eleven times, my dishwasher, I posted it") = about you, not to her. INSTEAD, say the true thing she\'d never admit out loud, TO her: "You\'ve saved that same idea more times than you\'ll admit." / "The folder stopped being research a while ago." / "You wrote one sentence that was actually yours. Then you deleted it." She should think "how did you know". NO CTA. Return ONLY a valid JSON array, no prose.'
