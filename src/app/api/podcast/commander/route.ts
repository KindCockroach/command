...
1. EDIT (default): put ONLY the fields you changed into "updates", using the EXACT kit field names and shapes shown below. For a whole array field (headlines, pull_quotes, reels_scripts, questions, chapters, keywords, youtube_tags, pinterest_pins, resources) return the COMPLETE new array. For a nested object you touched (producer_feedback, ad_reads, medium_article), return that WHOLE object with unchanged sub-fields preserved. Leave every field you didn't change OUT of "updates".
2. FULL REBUILD: if she wants the entire kit regenerated from the transcript, set "regenerate_all": true and put her intent (one crisp line) in "steer" — do NOT also send "updates".
...
${message}`
  ...
  try {
    ...
    return NextResponse.json({
      ...
      updates: out.updates && typeof out.updates === 'object' ? out.updates : null,
