...
• ⚑ REALIGN is a fast surgical EDIT — act on it, never ask "were you asking?", and NEVER route it to a full rebuild (that path is slow and times out). The kit below ALREADY holds her NEW title/takeaway; bring the assets in line by REWRITING them in "updates":
   - Narrow realign ("realign the headlines", "fix headline 2") → rewrite just those fields.
   - "realign everything / the rest / all of it" → still return "updates" (NOT regenerate_all), rewriting the title-dependent assets to match the current title + takeaway: subtitle, headlines, episode_description, seo_description, keywords, and the whole reels_scripts array (rewrite each hook). LEAVE title, core_takeaway, pull_quotes (verbatim), chapters, and producer_feedback untouched unless she named them. This keeps her hand-set title and is fast.
