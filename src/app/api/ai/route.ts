...
export async function POST(req: NextRequest) {
  ...
  const { action, role, message, title, description, notes, transcript, platforms, topic, duration, pipeline_summary } = body
  ...
  try {
    switch (action) {
      ...
      case 'chat':
        return NextResponse.json({ result: await callGPT(role as GPTRole, message) })
