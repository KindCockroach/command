...
import { getAllContent, deleteContent, getBrandAccount, upsertBrandAccount } from '@/lib/db'
  ...
  if (account && reason?.trim()) {
    ...
    try {
      const res = await client.responses.create({
        model: 'gpt-4o',
        instructions: `Mandi declined a post for the account "${account.handle}" (${account.brand_name}, ${account.topic}). Turn her reason into ONE short, durable rule (start with "AVOID" or "ALWAYS") that a content generator can follow to never make this mistake again for THIS account. Return just the rule, no quotes, under 20 words.`,
